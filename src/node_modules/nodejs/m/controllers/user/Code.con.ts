...
class Controller extends potato.Controller{
    ...
    Item(request: potato.Request, resolve: (data: Object | potato.ViewRendererData) => void, reject: (error: potato.PError) => void){
        let closeButton: potato.Request = new potato.Request(request.root, 'm/common/CloseButton', 'Item');
        let data = request.createViewRenderData({CloseButton:closeButton});
        let template = new potato.View('m/user/CodeTpl');
